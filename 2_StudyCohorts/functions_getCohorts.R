...
do_exclusion <- function(cdm, cohort, id, S_start_date) {
  ...
  cohort <- cohort %>%
    filter(is.na(.data$date_previous) | .data$date_previous < -42) %>% compute()
  ...
  attrition <- rbind(attrition,
    ...
    cohort <- cohort %>% filter(.data$prior_history >= 365) %>% compute()
    attrition <- rbind(attrition, 
                       dplyr::tibble(number_observations = cohort %>%
                                       dplyr::tally()
    ...
    cohort <- cohort %>%
      addCohortIntersectDays(cdm, targetCohortTable = InitialCohortsName, targetCohortId = 36, window = list(c(-90,-1)), order = "last", nameStyle = "last_flu") %>%
      compute()
